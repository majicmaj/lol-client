  ...
  submit = () => {
    ...
    if (
      ...
      Axios.post("https://lolbuilder.herokuapp.com/team/create", {
        ...
        creation: Date(Date.now()).toString()
      })
        .then(res => {
          console.log(res);
      ...
      champsicon = this.props.champions.map(each => {
        return (
          ...
          >
            ...
            <p className="champStats">
              A{each.attack},D{each.defense},M{each.magic}
            </p>
          </div>
        );
      });
    ...
    return (
      <main className="team_main">
        <div className="team_builder">
          <h1>Team builder</h1>
          <p>
            Click on your desired champion's icons and then click on the
            position you would like to place them.
          </p>
        ...
        <div className="team_picker">{champsicon}</div>
      </main>
    );
  }
}
